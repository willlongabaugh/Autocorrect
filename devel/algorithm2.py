...
# Minor Issue: apostrophes in edit distance 2

#I commented the code, and added code to handle single letters (but it doesnt work because of something in the front end?)
#added code to deal with double words
...
class algorithm():
    ...
    def findCandidates(self,w):
        ...
        if len(self.candidates) < self.size:  #check to see if the input is two words back to back "elephantpelican"
            for x in range(0,len(w) - 1): #check every spot in the input
                ...
                if node.end and word:
                    ...
                    if node2.end and word:  #add to the trie
                        self.candidates.append(((node,node2),2))

            for x in range(0,len(w) - 2):  #input has a bottom row letter in between two words "elephantvpelican"
                node = self.trie.header
                word = True
